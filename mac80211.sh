...
mac80211_hostapd_setup_base() {
	...
	case "$htmode" in
		...
		HT40*|VHT40|VHT80|VHT160)
			case "$hwmode" in
				a)
					if [ "$channel" -lt 180 ]; then
						case "$(( ($channel / 4) % 2 ))" in
							1) ht_capab="[HT40+]";;
							0) ht_capab="[HT40-]";;
						esac
					else
						#support channel pair (184, 188) and (192, 196)
						case "$(( ($channel / 4) % 2 ))" in
							1) ht_capab="[HT40-]";;
							0) ht_capab="[HT40+]";;
						esac
...
${max_all_num_sta:+max_all_num_sta=$max_all_num_sta}
$base_cfg
...
drv_mac80211_wpaupdate() {

	for_each_interface "sta" mac80211_wpa_update
